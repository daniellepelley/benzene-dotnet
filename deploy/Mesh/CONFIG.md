...
| `auth.dispatchRole` | string? | `null` | Optional | When set (and `dispatch.enabled` is `true`), intended to additionally require this role/group for `mesh:dispatch` specifically. **Bound and validated but not yet enforced** — see `README.md`'s note on why `mesh:dispatch` has no reachable HTTP route yet. |
