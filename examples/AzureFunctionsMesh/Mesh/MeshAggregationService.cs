...
/// <summary>
/// One discovery + aggregation pass: discover the benzene-tagged Azure resources (Web Apps and/or
/// Function Apps), write the discovered registry to Blob Storage, interrogate each over HTTPS, and
/// write the catalog artifacts. Shared by the on-demand <c>POST /mesh/refresh</c> endpoint and the
/// <see cref="AggregateTimerFunction"/> timer trigger.
/// </summary>
public class MeshAggregationService
{
    ...
    // Single-writer gate: the timer pass and an on-demand POST /mesh/refresh both call RunAsync against
    // the same remote Blob store. Without this, two overlapping passes can interleave their writes
