...
/// the incoming <see cref="OrderCreatedEvent"/> onto the same Service Bus "orders" queue the
/// ServiceBusFunction ingress trigger consumes from (see <see cref="DependenciesBuilder"/>'s
/// <c>AddOutboundRouting</c> wiring), via <see cref="IBenzeneMessageSender"/> - the same
/// transport-agnostic port a generated client uses. Reachable like any other handler (HTTP here) to
/// keep the demo self-contained; in a real service this call would typically live at the end of
/// <c>CreateOrderMessageHandler</c> or similar, right after the order is actually created.
/// </summary>
