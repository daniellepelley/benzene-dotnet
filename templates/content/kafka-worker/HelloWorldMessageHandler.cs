...
// shape for a Kafka consumer, since nothing is written back. Benzene routes each record to a handler by
// matching its topic against [Message("...")] - there's no colon-separated topic-id convention here the
// way there is for HTTP/SQS/SNS, so whatever you pass in [Message(...)] must be exactly the Kafka topic
// string. The handler takes its collaborators (here IGreeter) via the constructor - they're resolved
// from DI, and a test can swap them for a stand-in (see BenzeneStarter.Tests). Keeping side effects
// behind an injected service like this is what makes the handler straightforward to test.
[Message("hello_world")]
