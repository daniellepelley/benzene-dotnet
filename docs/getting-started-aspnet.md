...
This guide takes you from an empty folder to a running Benzene service on **ASP.NET Core** in about five
minutes — no cloud account required. Reach for this host when you're building a plain web app or API, or
running in a container that speaks HTTP. Deploying to Lambda or Functions instead? Start with
...
- Each handler is mapped to a **topic** — a stable string like `hello:world` that identifies the
  operation. Handlers are discovered automatically by reflection, so there's no routing table to maintain.
- A **transport** (ASP.NET Core here; AWS Lambda or Azure Functions elsewhere) is wired up in a
  **middleware pipeline** that turns an incoming request into a message, routes it to the matching handler
  by topic, and turns the result back into a transport-native response.
...
- `[Message("hello:world")]` maps the handler to its topic. Every Benzene transport routes by topic, so
  this is the identifier that stays constant across HTTP, Lambda, SQS, and the rest.
- `[HttpEndpoint("GET", "/hello/{name}")]` maps an HTTP method and path onto that same topic. The `{name}`
  segment is bound onto the request's `Name` property.
...
- **Add correlation & logging** — trace requests end-to-end with [Correlation IDs](correlation-ids.md) and
...
- **Move to the cloud** — the same handler runs on [AWS Lambda](getting-started-aws.md) (API Gateway, SQS,
  SNS, EventBridge) or [Azure Functions](azure-functions.md).
- **Go deeper with recipes** — the [Cookbooks](cookbooks/README.md) cover retries, fan-out, caching, and
  distributed tracing.
