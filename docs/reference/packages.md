...
| `Benzene.Aws.Lambda.DynamoDb` | Handle DynamoDB Streams events — `UseDynamoDb(...)`. |
| `Benzene.Aws.Lambda.Kinesis` | Handle Kinesis Data Streams events — `UseKinesisStream(...)`. |
| `Benzene.Aws.Lambda.EventBridge` | Handle EventBridge events — `UseEventBridge(...)`. |
| `Benzene.Aws.Lambda` | Umbrella meta-package: references the Lambda core plus every event-source transport above, so a service can add one package instead of naming each. Reference the individual packages for a narrower dependency. |
...
| `Benzene.Kafka.Streaming` | A self-hosted, windowed and checkpointed Kafka stream consumer (`BenzeneKafkaStreamWorker<TKey,TValue>`, `worker.UseKafkaStream<TKey,TValue>(...)`) that accumulates consumed records into batches (by size or by age) and runs each batch through a fan-in streaming pipeline with real **per-topic-partition** offset checkpointing — distinct from `Benzene.Kafka.Core`'s `UseKafka`, which dispatches one message handler *per record*. |
...
| `Benzene.Clients.Http` | HTTP client middleware for sending outbound HTTP requests through the Benzene client pipeline. |
