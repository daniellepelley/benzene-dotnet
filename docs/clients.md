...
| `.UseCorrelationId(correlationKey = null)` (`Benzene.Clients.CorrelationId`) | `CorrelationIdMiddleware` | Stamps the current `ICorrelationId.Get()` value onto `OutboundContext.Headers` under `x-correlation-id` by default (`CorrelationHeaderDefaults.HeaderKey`) — unless a header under the same key is already present (an explicit per-call header wins). See [Correlation IDs](correlation-ids.md). |
...
It wraps the topic, headers, and serialized message body into its own envelope, `BenzeneMessageClientRequest`, and invokes with that as the payload. An exception during invocation is caught and returned as `BenzeneResult.ServiceUnavailable<TResponse>()` — including the request/response shape's `AwsLambdaFunctionErrorException`, thrown by the lower-level `AwsLambdaClient` when `InvokeResponse.FunctionError` is set (AWS returns HTTP 200 even when the target function threw), so a function error is never mistaken for a successful response here either. The fire-and-forget (`Event`) shape is classified the same way: `AwsLambdaClient` throws `AwsLambdaEventInvokeFailedException` when `InvokeResponse.StatusCode` is not 2xx (e.g. a throttling or validation error surfaced synchronously by the Invoke API), so that too is caught here and reported as `ServiceUnavailable` rather than unconditionally `Accepted` — mirroring `LambdaContextConverter<T>.MapResponseAsync`'s classification below for the `.UseAwsLambda<T>()` pipeline shape.
