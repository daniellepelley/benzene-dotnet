...
| `.UseCorrelationId(correlationKey = "correlationId")` (`Benzene.Clients.CorrelationId`) | `CorrelationIdMiddleware` | Stamps the current `ICorrelationId.Get()` value onto `OutboundContext.Headers` — unless a header under the same key is already present (an explicit per-call header wins). See [Correlation IDs](correlation-ids.md). |
