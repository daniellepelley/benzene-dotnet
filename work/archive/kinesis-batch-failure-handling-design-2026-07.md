> ARCHIVED 2026-08-20: actioned; implemented in full in `src/Benzene.Aws.Lambda.Kinesis` (`KinesisBatchResponse`, `KinesisStreamCheckpointer`).
...
`work/archive/batch-failure-handling-2026-07.md`'s "Deliberately deferred" table (written 2026-07-16) said Kinesis
...
`work/archive/streaming-plan-2026-08.md` directly: **that estimate was made without accounting for work
...
keyed on `SequenceNumber` is future (streaming Phase 2, see `work/archive/streaming-plan-2026-08.md`)."* This
...
This isn't Kinesis-specific scaffolding - `work/archive/streaming-plan-2026-08.md`'s own Phase 2 already
...
`work/archive/batch-failure-handling-2026-07.md`)? **Recommendation: catch it in `KinesisStreamApplication` and
...
- **Handlers must opt into calling `Checkpointer.CheckpointAsync(record)`** at the right point in
  ...
  `work/archive/streaming-plan-2026-08.md` Phase 2, not reinvented here. Recommend implementing that operator
...
- **DynamoDB Streams** (`Benzene.Aws.Lambda.DynamoDb`) has the identical `ReportBatchItemFailures`
  ...
  family) - per `work/archive/batch-failure-handling-2026-07.md`, `DynamoDbApplication` is a deliberately different,
...
5. Update `Benzene.Aws.Lambda.Kinesis/CLAUDE.md`'s "Checkpointing uses... NullStreamCheckpointer"
   line, `work/archive/batch-failure-handling-2026-07.md` (move Kinesis from "Deliberately deferred" to a new
   shipped-table row), and `work/archive/streaming-plan-2026-08.md`'s Phase 2 (mark the Kinesis half of
   real-checkpoint-wiring done, leaving SQS streaming as the still-open half of that phase).
6. Consider implementing `UseCheckpointAfterEach()` (`work/archive/streaming-plan-2026-08.md` Phase 2) in the
