> ARCHIVED 2026-08-20: actioned; executed plan — streaming shipped (`src/Benzene.Aws.Lambda.Kinesis`/`.DynamoDb`, `Benzene.Core.Middleware` streaming); the Kinesis follow-up landed via `kinesis-batch-failure-handling-design-2026-07.md` (archived here).
...
- `MessageRouter<TContext>` resolves the request lazily via
  ...
  `work/archive/grpc-enhancement-plan-2026-08.md`). This is the basis for the optional handler bridge (Phase 3).
...
- **Phase 2 — stream solutions.** `UseWindow`, `UsePartitionedBy`, checkpoint steps, and real
  ...
  `work/archive/kinesis-batch-failure-handling-design-2026-07.md`. This also added
...
- The design deliberately mirrors how gRPC streaming is approached in
  `work/archive/grpc-enhancement-plan-2026-08.md` (streaming via `IAsyncEnumerable` request/response types), so
