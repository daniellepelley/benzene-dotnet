...
4. Pre-1.0 compatibility posture is the **topic-prefix precedent** (`work/topic-prefix-migration.md`):
...
- **benzene-dotnet conventions** are as in `work/spec-mesh-tooling-implementation-plan.md` §0
...
- Client-side consumption patterns for the ports (typed accessors, generated clients) belong to
  the cross-language client work — see `work/cross-language-clients-plan.md` (parallel plan;
...
- **Retry semantics and dead-letter policy** — the problem document *describes* a failure; what a
  ...
  `docs/plans/results-taxonomy-plan.md` D4).
