...
`work/lightweight-non-http-transport-design.md` (which solves the opposite problem — making
cross-container calls cheaper) and `work/benzene-clients-redesign-plan.md` (whose
...
- **`OutboundRoutingBuilder`/`IBenzeneMessageSender`/`OutboundContext`** (shipped per
  `work/benzene-clients-redesign-plan.md`) is the outbound registration surface `.UseInProcess()`
