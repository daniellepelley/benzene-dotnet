...
- **Methodology gap, closed as instructed:** re-ran the search by *shape* (a constructor-stored
  ...
  - WP-3's #192 fix had not actually landed on this branch despite being recorded as shipped in the
  round 12-13 ruling; `HttpBenzeneMessageClient` (the ninth) was already safe (nullable `ILogger?`,
  used via `?.` throughout). Fixed all eight alongside #266/#267 - same mechanical, additive,
  behavior-preserving change, and the family this WP is titled for ("Clients null-logger
  completion"). **The re-grep also found the identical hazard shape in twelve classes outside the
